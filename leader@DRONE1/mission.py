...
class MissionManager:
    ...
    def upload_mission(self, altitude=5.0):
        ...
        if not self.waypoints:
            print("[Mission] No waypoints.")
            return False
            
        print(f"[Mission] Uploading {len(self.waypoints)} waypoints + TAKEOFF + LAND...")
        ...
        print(f"[Mission] Sending TAKEOFF to {altitude}m")
        ...
        for i, (lat, lon) in enumerate(self.waypoints):
            seq = i + 1
            print(f"[Mission] Sending WP {seq}: {lat}, {lon}")
            self.backend.master.mav.mission_item_int_send(
                self.backend.master.target_system, 
                self.backend.master.target_component,
                seq, 
        ...
        print(f"[Mission] Sending LAND item at seq {seq_land}")
    ...
    def _run_guided_mission(self, altitude):
        print(f"[Mission] ▶️ STARTING MISSION with {len(self.waypoints)} Waypoints")
        
        # 1. Set Mode GUIDED
        print("[Mission] Switching to GUIDED...")
        ...
        # 3. Iterate Waypoints
        for i, (lat, lon) in enumerate(self.waypoints):
            print(f"[Mission] 📍 Heading to WP {i+1}...")
            ...
            while True:
                ...
                # Debug print limited to every 2 seconds could be better, but simple is ok
        ...
        # 4. Land
        print("[Mission] 🏁 Mission Complete. Landing...")
