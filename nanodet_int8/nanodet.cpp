...
NanoDet::NanoDet(const char* param, const char* bin, bool useGPU)
{
    ...
    // ── Maximum RPi4 performance (4x Cortex-A72) ──────────────
    this->Net->opt.num_threads = 4;             // Use all 4 cores
    this->Net->opt.use_fp16_arithmetic = true;   // FP16 NEON acceleration
    this->Net->opt.use_fp16_storage = true;      // FP16 memory (saves bandwidth)
    this->Net->opt.use_packing_layout = true;    // NEON-optimized memory layout
    this->Net->opt.lightmode = true;             // Release intermediate blobs ASAP
    this->Net->opt.use_winograd_convolution = true; // Fast convolution algorithm
...
void NanoDet::loadCalibration(const char* calib_path)
{
    cv::FileStorage fs(calib_path, cv::FileStorage::READ);
    if (!fs.isOpened()) {
        fprintf(stderr, "Failed to open calibration file: %s (Pose estimation disabled)\n", calib_path);
        return;
    }
    ...
    // Read calibration image resolution
    int calib_w = 0, calib_h = 0;
    fs["image_width"] >> calib_w;
    fs["image_height"] >> calib_h;
    fs.release();

    // Scale intrinsics to match capture resolution (640x480)
    // Calibration was done at calib_w x calib_h (e.g. 1280x720)
    if (calib_w > 0 && calib_h > 0) {
        float scale_x = 640.0f / (float)calib_w;
        float scale_y = 480.0f / (float)calib_h;
        this->fx *= scale_x;
        this->fy *= scale_y;
        this->cx *= scale_x;
        this->cy *= scale_y;
        printf("[Calibration] Scaled from %dx%d to 640x480\n", calib_w, calib_h);
    }

    if (this->fx > 0 && this->fy > 0) {
        this->has_calibration = true;
        printf("[Loaded Calibration]: fx=%.1f  fy=%.1f  cx=%.1f  cy=%.1f\n", this->fx, this->fy, this->cx, this->cy);
    } else {
        fprintf(stderr, "[Error] Invalid calibration data in %s\n", calib_path);
    }
}

bool NanoDet::estimatePose(const BoxInfo& bbox, float& x, float& y, float& z)
{
    // Pose estimation is implemented directly in draw_bboxes to use original image coordinates
    return false;
}
